...
describe('CacheKeyResolver', () => {
    ...
    it('only expires cache key if number of unique calls exceeds max size', () => {
        const resolver = new CacheKeyResolver({ maxSize: 2 });
    ...
    it('does not expire cache key if number of unique calls does not exceed max size', () => {
        const resolver = new CacheKeyResolver({ maxSize: 3 });

        expect(resolver.getKey('hello', 'world')).toEqual('1');
        expect(resolver.getKey('foo', 'bar')).toEqual('2');
        expect(resolver.getKey('hello', 'bye')).toEqual('3');

        // This call should return the same key because the number of unique
        // calls is under the max size
        expect(resolver.getKey('hello', 'world')).toEqual('1');
    });
