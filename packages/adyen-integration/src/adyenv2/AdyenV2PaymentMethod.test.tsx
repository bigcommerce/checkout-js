...
describe('when using Adyen V2 payment', () => {
  ...
  describe('#During payment', () => {
    it('renders 3DS modal if required by selected method', async () => {
      ...
      const { container } = render(<PaymentMethodTest {...defaultAdyenProps} method={method} />);
    ...
    it('do not render 3DS modal if required by selected method', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      const initializeOptions: PaymentInitializeOptions = (initializePayment as jest.Mock).mock
        .calls[0][0];
    ...
    it('cancels 3DS modal flow if user chooses to close modal', async () => {
      ...
      act(() => {
        initializeOptions.adyenv2.additionalActionOptions.onLoad(cancelAdditionalActionModalFlow);
      });
