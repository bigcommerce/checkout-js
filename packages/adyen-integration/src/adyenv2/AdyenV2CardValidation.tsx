...
export type FieldsValidation = Partial<Record<AdyenCardFields, AdyenValidationState>>;
  ...
  useEffect(() => {
    ...
    if (
      ...
      (!fieldsValidation[cardValidationState.fieldType] ||
        fieldsValidation[cardValidationState.fieldType]?.valid !== cardValidationState.valid)
    ) {
  ...
  return (
    <div>
      {shouldShowNumberField && (
        <p>
          <strong>
            {language?.translate('payment.instrument_trusted_shipping_address_title_text')}
          </strong>
