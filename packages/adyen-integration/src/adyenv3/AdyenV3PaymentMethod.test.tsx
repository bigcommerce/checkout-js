...
describe('when using AdyenV3 payment', () => {
    ...
    describe('paymentProviderRadio', () => {
        it('syncs paymentProviderRadio when Adyen method is selected', () => {
            const defaultAdyenProps: PaymentMethodProps = {
                method,
                onUnhandledError: jest.fn(),
                checkoutService,
            ...
            render(
                <CheckoutProvider checkoutService={checkoutService}>
                    <PaymentFormProvider paymentForm={paymentForm}>
                        <LocaleContext.Provider value={localeContext}>
                            <Formik
                                initialValues={{ paymentProviderRadio: 'adyenv3-scheme' }}
                                onSubmit={noop}
                            >
                                <AdyenV3PaymentMethod {...defaultAdyenProps} />
                            </Formik>
                        </LocaleContext.Provider>
                    </PaymentFormProvider>
                </CheckoutProvider>,
        ...
        it('does not sync paymentProviderRadio when another payment method is selected', () => {
            const defaultAdyenProps: PaymentMethodProps = {
                method,
                onUnhandledError: jest.fn(),
                checkoutService,
            ...
            jest.mocked(paymentForm.setFieldValue).mockClear();
            ...
            render(
                <CheckoutProvider checkoutService={checkoutService}>
                    <PaymentFormProvider paymentForm={paymentForm}>
                        <LocaleContext.Provider value={localeContext}>
                            <Formik
                                initialValues={{ paymentProviderRadio: 'authorizenet' }}
                                onSubmit={noop}
                            >
                                <AdyenV3PaymentMethod {...defaultAdyenProps} />
                            </Formik>
