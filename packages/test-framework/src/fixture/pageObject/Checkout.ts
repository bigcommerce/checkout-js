...
export class Checkout {
  ...
  async completeCustomerStepAsGuest(email = `test@example.com`): Promise<void> {
    await this.page.locator('[data-test="checkout-customer-guest"]').waitFor({ state: 'visible' });
  ...
  async completeCustomerStep(email: string, password: string): Promise<void> {
    ...
    await this.page.locator('[data-test="checkout-customer-returning"] #password').fill(password);
  ...
  async fillAddressForm({
    ...
    await this.page.locator(`${formId} [data-test="postCodeInput-text"]`).fill(address.postalCode);
  }
  ...
  async completeShippingAddressForm(
    ...
    await this.page.locator('[data-test="shipping-address-heading"]').waitFor({ state: 'visible' });
  ...
  async completeBillingAddressForm(address: Address): Promise<void> {
    await this.page.locator('[data-test="billing-address-heading"]').waitFor({ state: 'visible' });
