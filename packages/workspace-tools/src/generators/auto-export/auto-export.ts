...
export interface AutoExportOptions {
    ...
    componentRegistryOutputPath?: string;
}
...
export default async function autoExport({
    ...
    if (createComponentRegistry) {
        ...
        // Write component registry to separate file if path is provided
        if (componentRegistryOutputPath) {
            const registryDir = path.dirname(componentRegistryOutputPath);
            ...
            fs.writeFileSync(componentRegistryOutputPath, componentRegistry, 'utf8');
        }
    }
...
function generateComponentRegistryTypeScript(componentRegistry: Record<string, unknown[]>): string {
    const entries = Object.entries(componentRegistry)
        .sort(([a], [b]) => a.localeCompare(b)) // Sort by component name for deterministic output
