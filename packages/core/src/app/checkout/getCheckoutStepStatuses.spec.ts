import {createCheckoutService,
  CheckoutSelectors,
  CheckoutService,} from '@bigcommerce/checkout-sdk';
import { find } from 'lodash';

import {getAddressFormFields,
  getAddressFormFieldsWithCustomRequired,} from '../address/formField.mock';
...
describe('getCheckoutStepStatuses()', () => {
  ...
  describe('billing step', () => {
    ...
    describe('amazonpay', () => {
      it('is marked as complete if billing address is not provided', () => {
        ...
        expect(find(steps, { type: CheckoutStepType.Billing })).toHaveProperty('isComplete', true);
      });
      ...
      it('is marked as complete if billing address is not provided and custom fields are valid', () => {
        ...
        jest
          .spyOn(state.data, 'getBillingAddressFields')
      ...
      it('is marked as incomplete if billing address is not provided but custom fields are invalid', () => {
        ...
        expect(find(steps, { type: CheckoutStepType.Billing })).toHaveProperty('isComplete', false);
      });
      ...
      it('is marked as non-editable if step is complete and there are no custom fields', () => {
        ...
        expect(find(steps, { type: CheckoutStepType.Billing })).toHaveProperty('isEditable', false);
      });

      it('is marked as editable if step is complete and there is custom fields', () => {
        jest.spyOn(state.data, 'getCheckout').mockReturnValue(getCheckoutWithAmazonPay());
        jest.spyOn(state.data, 'getBillingAddress').mockReturnValue(getBillingAddress());
        jest.spyOn(state.data, 'getBillingAddressFields').mockReturnValue(getAddressFormFields());

        const steps = getCheckoutStepStatuses(state);

        expect(find(steps, { type: CheckoutStepType.Billing })).toHaveProperty('isEditable', true);
      });
    });
  });
  ...
  describe('shipping step', () => {
    ...
    it('is marked as not editable if cart does not contain physical items', () => {
      ...
      expect(
        find(getCheckoutStepStatuses(state), { type: CheckoutStepType.Shipping })!.isEditable,
      ).toBe(true);
  ...
  it('does not mark incomplete step as active if it is not required', () => {
    ...
    expect(find(getCheckoutStepStatuses(state), { type: CheckoutStepType.Billing })!.isActive).toBe(
      false,
    );
