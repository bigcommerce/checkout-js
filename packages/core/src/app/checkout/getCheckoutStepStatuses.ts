...
const getCustomerStepStatus = createSelector(
  ...
  (checkout, customer, billingAddress, config, cart, paymentProviderCustomer) => {
    const hasEmail = !!(customer?.email || billingAddress?.email);
    const isUsingWallet = checkout?.payments
...
const getBillingStepStatus = createSelector(
  ...
  (checkout, billingAddress, billingAddressFields) => {
    const hasAddress = billingAddress
      ...
      : false;
    const isUsingWallet = checkout?.payments
      ? checkout.payments.some((payment) => SUPPORTED_METHODS.includes(payment.providerId))
      : false;
    const isComplete = hasAddress || isUsingWallet;
    const isUsingAmazonPay = checkout?.payments
    ...
    const isUsingGooglePay = checkout?.payments
    ...
    const isUsingPaypal = checkout?.payments
