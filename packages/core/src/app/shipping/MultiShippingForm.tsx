...
const MultiShippingForm: FunctionComponent<MultiShippingFormProps> = ({
  ...
  const {
    checkoutSettings: { enableOrderComments: shouldShowOrderComments, shippingQuoteFailedMessage },
  } = config;
  ...
  const renderAllocatedBanner = (shippableItemsCount: number): ReactNode => {
    if (shippableItemsCount > 0) {
      return (
        <Alert additionalClassName={themeV2 ? 'body-regular' : ''} type={AlertType.Info}>
          <TranslatedString
    ...
    return (
      <Alert additionalClassName={themeV2 ? 'body-regular' : ''} type={AlertType.Success}>
