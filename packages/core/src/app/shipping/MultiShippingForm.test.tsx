...
describe('MultiShippingForm Component', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(checkoutState.data, 'getBillingAddressFields')
      .mockReturnValue(getAddressFormFields());

    jest
      .spyOn(checkoutState.data, 'getShippingAddressFields')
      .mockReturnValue(getAddressFormFields());
  ...
  it('shows error when creating a new destination if destination 1 is incomplete.', async () => {
    ...
    expect(
      screen.getByText(
        localeContext.language.translate('shipping.multishipping_incomplete_consignment_error', {
          consignmentNumber: 1,
        }),
      ),
    ).toBeInTheDocument();
  ...
  it('adds new shipping destination and open allocate items modal and validate quantity input', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(
          localeContext.language.translate('shipping.multishipping_no_item_allocated_message'),
        ),
      ).not.toBeInTheDocument();
    });
    ...
    expect(
      screen.queryByText(
        localeContext.language.translate('shipping.multishipping_digital_item_no_shipping_banner'),
      ),
    ).not.toBeInTheDocument();
  ...
  it('displays all items are allocated banner', async () => {
    ...
    expect(
      screen.getByText(
        localeContext.language.translate('shipping.multishipping_all_items_allocated_message'),
      ),
    ).toBeInTheDocument();
  });
  ...
  it('removes a consignment', async () => {
    jest
      .spyOn(checkoutService, 'deleteConsignment')
  ...
  describe('NewConsignment empty cart error handling', () => {
    it('calls onUnhandledError when empty cart error is thrown during item allocation', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            localeContext.language.translate('shipping.multishipping_consignment_index_heading', {
              consignmentNumber: 1,
            }),
          ),
        ).toBeInTheDocument();
