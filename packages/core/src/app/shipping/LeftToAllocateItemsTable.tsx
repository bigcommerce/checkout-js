...
const LeftToAllocateItemsTable: FunctionComponent<LeftToAllocateItemsTableProps> = ({
  items,
  formErrors,
}: LeftToAllocateItemsTableProps) => {
  ...
  return (
    <table className="table left-to-allocate-items-table">
      <thead>
        <tr>
          <th className={themeV2 ? 'body-medium' : ''}>
      ...
      <tbody>
        {items.map((item) => (
          <LeftToAllocateItem error={formErrors[item.id.toString()]} item={item} key={item.id} />
        ))}
      </tbody>
    </table>
  );
