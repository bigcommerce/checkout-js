...
describe('Shipping Component', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(checkoutService, 'loadShippingAddressFields')
      .mockResolvedValue({} as CheckoutSelectors);

    jest.spyOn(checkoutService, 'loadShippingOptions').mockResolvedValue({} as CheckoutSelectors);
    ...
    jest
      .spyOn(checkoutState.data, 'getCustomer')
  ...
  it('does not render ShippingForm while initializing', () => {
    jest
      .spyOn(checkoutService.getState().statuses, 'isLoadingShippingCountries')
  ...
  it('calls only navigateNextStep when no changes', async () => {
    jest
      .spyOn(checkoutService.getState().data, 'getShippingAddress')
    ...
    describe('when there are multiple consignments', () => {
      beforeEach(async () => {
        jest
          ...
          .mockReturnValue([getConsignment(), getConsignment()]);
