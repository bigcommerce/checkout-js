...
const StripeShippingAddress: FunctionComponent<StripeShippingAddressProps> = (props) => {
    ...
    useEffect(() => {
        if (consignments[0]) {
            ...
            if (
                ...
                !getRecommendedShippingOption(availableShippingOptions)
            ) {
                handleLoading();
            }
        }
    ...
    useEffect(() => {
        const hasStripeAddressAndHasShippingOptions =
            stripeShippingAddress.firstName && hasSelectedShippingOptions(consignments);
    ...
    const availableShippingList = countries?.map((country) => ({
        code: country.code,
        name: country.name,
    }));
    const allowedCountries = availableShippingList
        ? availableShippingList.map((country) => country.code).join(', ')
        : '';
    ...
    const handleStripeShippingAddress = useCallback(
        async (shipping: StripeShippingEvent) => {
            const {
                complete,
                phoneFieldRequired,
                value: {
            ...
            if (complete) {
                ...
                const country = availableShippingList?.find(
