...
describe('Shipping component', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(checkoutService, 'loadBillingAddressFields')
      .mockResolvedValue({} as CheckoutSelectors);
  ...
  describe('when new multishipping ui is enabled', () => {
    ...
    it('opens information dialog on clicking `ship to multiple address` when promotional item is present in the cart', async () => {
      ...
      await userEvent.click(
        within(confirmationModal).getByText(localeContext.language.translate('common.back_action')),
      );
    ...
    it('opens information dialog on mount and when multishipping mode is ON and promotional item is present in the cart', async () => {
      ...
      expect(
        within(confirmationModal).getByText(
          localeContext.language.translate('shipping.checkout_switched_to_single_shipping'),
        ),
      ).toBeInTheDocument();

      await userEvent.click(
        within(confirmationModal).getByText(localeContext.language.translate('common.ok_action')),
      );
