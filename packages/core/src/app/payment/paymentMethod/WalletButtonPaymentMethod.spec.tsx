...
describe('WalletButtonPaymentMethod', () => {
  ...
  describe('when user is signed into their payment method account and credit card is selected from their wallet', () => {
    ...
    it('displays information of selected credit card', () => {
      ...
      expect(component.find('[data-test="payment-method-wallet-card-name"]').text()).toContain(
        'Test Tester',
      );

      expect(component.find('[data-test="payment-method-wallet-card-type"]').text()).toContain(
        'Visa: **** 1111',
      );

      expect(component.find('[data-test="payment-method-wallet-card-expiry"]').text()).toContain(
        '10/22',
      );
    });
    ...
    it('displays card information in `accountNum` format', () => {
      ...
      const component = mount(<WalletButtonPaymentMethodTest { ...defaultProps } method={ method } />);
    ...
    it('signs out from payment method account of user when clicking on sign out link', async () => {
      ...
      const component = mount(
        <WalletButtonPaymentMethodTest { ...defaultProps } onSignOutError={ handleSignOutError } />,
      );
    ...
    it('notifies parent component if unable to sign out', async () => {
      ...
      jest.spyOn(checkoutService, 'signOutCustomer').mockRejectedValue(new Error('Unknown error'));
