  ...
  return (method) => {
    ...
    > = {
      ...
      [PaymentMethodId.PaypalCommerce]: {
        ...
        titleText: '',
        subtitle: (props: PaymentMethodSubtitleProps) => (
          <PaypalCommerceCreditBanner containerId="paypal-commerce-banner-container" {...props} />
        ),
      },
    ...
    if (method.gateway === PaymentMethodId.BlueSnapDirect) {
      ...
      if (method.id === 'ecp') {
        return {
          logoUrl: '',
          titleText: language.translate('payment.bluesnap_direct_electronic_check_label'),
        };
      }
...
> = ({
  ...
  const getSubtitle = () => {
    const node =
      subtitle instanceof Function ? subtitle({ onUnhandledError, methodId: method.id }) : subtitle;
