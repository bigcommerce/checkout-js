...
const PaymentSubmitButtonText: FunctionComponent<PaymentSubmitButtonTextProps> = memo(
  ({ methodId, methodName, methodType, methodGateway, initialisationStrategyType, brandName }) => {
        ...
        <TranslatedString
          id={
            methodId === PaymentMethodId.BraintreeVenmo
              ? 'payment.braintreevenmo_continue_action'
    ...
    if (methodType === PaymentMethodType.PaypalCredit) {
      return (
        <TranslatedString
          data={ { brandName } }
          id={
            brandName ? 'payment.continue_with_brand' : 'payment.paypal_pay_later_continue_action'
          }
        />
      );
    }
