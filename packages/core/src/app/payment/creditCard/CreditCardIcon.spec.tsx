...
import {IconCardAmex,
  ...
  IconCardTroy,
  IconCardUnionPay,
  IconCardVisa,} from '../../ui/icon';

import CreditCardIcon from './CreditCardIcon';

describe('CreditCardIcon', () => {
  it('returns American Express card icon', () => {
    expect(shallow(<CreditCardIcon cardType="american-express" />).find(IconCardAmex)).toHaveLength(
      1,
    );
  });
  ...
  it('returns Discover card icon', () => {
    expect(shallow(<CreditCardIcon cardType="discover" />).find(IconCardDiscover)).toHaveLength(1);
  });
  ...
  it('returns Maestro card icon', () => {
    expect(shallow(<CreditCardIcon cardType="maestro" />).find(IconCardMaestro)).toHaveLength(1);
  });

  it('returns Mastercard card icon', () => {
    expect(shallow(<CreditCardIcon cardType="mastercard" />).find(IconCardMastercard)).toHaveLength(
      1,
    );
  });

  it('returns Union Pay card icon', () => {
    expect(shallow(<CreditCardIcon cardType="unionpay" />).find(IconCardUnionPay)).toHaveLength(1);
  });
  ...
  it('returns Dankor card icon', () => {
    expect(shallow(<CreditCardIcon cardType="dankort" />).find(IconCardDankort)).toHaveLength(1);
  });
  ...
  it('returns Hipercard card icon', () => {
    expect(shallow(<CreditCardIcon cardType="hiper" />).find(IconCardHipercard)).toHaveLength(1);
  });
  ...
  it('returns default card icon if nothing matches', () => {
    expect(shallow(<CreditCardIcon cardType="foobar" />).hasClass('cardIcon-icon--default')).toBe(
      true,
    );
  });
  ...
  it('configures icon component with test ID', () => {
    ...
    expect(component.find(IconCardAmex).prop('testId')).toBe('credit-card-icon-american-express');
  });
});
