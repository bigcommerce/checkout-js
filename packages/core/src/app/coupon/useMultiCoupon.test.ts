...
describe('useMultiCoupon', () => {
  ...
  it('throws if checkout is not available', () => {
    ...
    expect(() => renderHook(() => useMultiCoupon())).toThrow('Checkout or order is not available');
  });
  ...
  describe('uiDetails', () => {
    it('returns correct uiDetails structure when consignments information is complete', () => {
      const checkout = {
        ...
        consignments: [
          {
            ...getConsignment(),
            lineItemIds: [getPhysicalItem().id.toString(), getDigitalItem().id.toString()],
          },
        ],
      };
    ...
    describe('discountItems', () => {
      ...
      it('includes all discount types when present', () => {
        ...
        expect(
          result.current.uiDetails.discountItems.some((item) => item.name === 'Save 10% (SAVE10)'),
        ).toBe(true);
      });
