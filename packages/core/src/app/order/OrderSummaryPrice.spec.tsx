...
describe('OrderSummaryPrice', () => {
  ...
  describe('when has non-zero amount', () => {
    ...
    describe('and has only required props', () => {
      ...
      it('renders additional elements/props', () => {
        ...
        expect(orderSummaryPrice.find('.cart-priceItem-currencyCode').text()).toMatch('(EUR)');

        expect(orderSummaryPrice.find('[data-test="cart-price-value-superscript"]').text()).toMatch(
          '*',
        );
      });
    });
  ...
  describe('when has zero amount', () => {
    ...
    describe('and no label', () => {
      beforeEach(() => {
        orderSummaryPrice = shallow(
          <OrderSummaryPrice amount={ amount } className="label" label="Label" testId="test" />,
        );
      });

      it('renders formatted amount', () => {
        expect(orderSummaryPrice.find(ShopperCurrency).props()).toMatchObject({ amount: 0 });
      });
    });
    ...
    describe('and zero label', () => {
      ...
      it('renders zero label', () => {
        expect(orderSummaryPrice.find('[data-test="cart-price-value"]').text()).toBe('Free');
      });
    });
  });
});
