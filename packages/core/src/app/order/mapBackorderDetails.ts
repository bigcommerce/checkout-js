...
export const mapBackorderDetails = (
    item: DigitalItem | PhysicalItem,
): Pick<OrderItemType, 'quantityBackordered' | 'quantityOnHand' | 'backorderMessage'> => {
    ...
    const quantityOnHand =
        ...
        (quantityBackordered != null ? item.quantity - quantityBackordered : undefined);
    const backorderMessage =
        item.stockPosition?.backorderMessage || item.backorderMessage || undefined;
