...
describe('ChecklistItemInput', () => {
  ...
  it('renders input as checked when is selected', () => {
    ...
    expect(component.find('.form-checklist-checkbox').prop('checked')).toBeTruthy();
  });
  ...
  it('renders input as unchecked when is not selected', () => {
    ...
    expect(component.find('.form-checklist-checkbox').prop('checked')).toBeFalsy();
  });

  it('calls onChange when input changes', () => {
    const onChange = jest.fn();
    const component = mount(
      <ChecklistItemInput isSelected={ true } name="foobar" onChange={ onChange } value="foobar_val" />,
    );
