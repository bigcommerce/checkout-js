...
describe('DynamicInput', () => {
  ...
  it('renders text input for default input with passed props', () => {
    expect(mount(<DynamicInputTest date={ date } id="field_33" name="x" />).html()).toMatchSnapshot();
  });
  ...
  it('renders number type input for number type', () => {
    expect(
      mount(<DynamicInputTest date={ date } fieldType={ DynamicFormFieldType.number } id="field_33" />)
        .find(TextInput)
        .prop('type'),
    ).toBe('number');
  });
  ...
  it('renders password type input for password type', () => {
    expect(
      mount(
        <DynamicInputTest date={ date } fieldType={ DynamicFormFieldType.password } id="field_33" />,
      )
        .find(TextInput)
        .prop('type'),
    ).toBe('password');
  ...
  it('renders tel type input for phone type', () => {
    expect(
      mount(
        <DynamicInputTest date={ date } fieldType={ DynamicFormFieldType.telephone } id="field_33" />,
      )
        .find(TextInput)
        .prop('type'),
    ).toBe('tel');
