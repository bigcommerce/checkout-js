import {createCheckoutService,
  CheckoutSelectors,
  CheckoutService,
  LineItemMap,} from '@bigcommerce/checkout-sdk';
...
describe('Billing Component', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(checkoutService.getState().data, 'getBillingAddressFields')
    ...
    jest
      .spyOn(checkoutService.getState().data, 'getBillingCountries')
      .mockReturnValue(getCountries());

    jest
      .spyOn(checkoutService.getState().statuses, 'isUpdatingBillingAddress')
  ...
  it('renders header', () => {
    expect(component.find('[data-test="billing-address-heading"]').text()).toBe('Billing Address');
  });
  ...
  it('does not render BillingForm while loading billing countries', () => {
    jest
      .spyOn(checkoutService.getState().statuses, 'isLoadingBillingCountries')
