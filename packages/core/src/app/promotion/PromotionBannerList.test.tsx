...
describe('PromotionBannerList', () => {
  it('renders list of promotion banners', () => {
    ...
    expect(
      within(promotionBanners[0]).getByText(promotions[0].banners[0].text),
    ).toBeInTheDocument();
    expect(
      within(promotionBanners[1]).getByText(promotions[0].banners[1].text),
    ).toBeInTheDocument();
  });
  ...
  it('renders nested promotion banners as flat list', () => {
    ...
    expect(
      within(promotionBanners[2]).getByText(promotions[0].banners[0].text),
    ).toBeInTheDocument();
    expect(
      within(promotionBanners[3]).getByText(promotions[0].banners[1].text),
    ).toBeInTheDocument();
  });
