...
describe('Customer', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(checkoutService.getState().data, 'getBillingAddress')
    ...
    jest.spyOn(checkoutService, 'initializeCustomer').mockResolvedValue(checkoutService.getState());
  ...
  describe('when view type is "guest"', () => {
    ...
    it('renders guest form if billing address is undefined', async () => {
      jest.spyOn(checkoutService.getState().data, 'getBillingAddress').mockReturnValue(undefined);
    ...
    it('continues checkout as guest and does not send consent if not required', async () => {
      jest
        .spyOn(checkoutService, 'continueAsGuest')
    ...
    it('changes to login view when "show login" event is received', async () => {
      ...
      const component = mount(
        <CustomerTest onChangeViewType={ handleChangeViewType } viewType={ CustomerViewType.Guest } />,
      );
    ...
    it('renders CancellableEnforcedLogin if continue as guest fails with code 403', async () => {
      jest.spyOn(checkoutService, 'continueAsGuest').mockRejectedValue({ status: 403 });
    ...
    it('renders EnforcedLogin form if continue as guest fails with code 429', async () => {
      jest.spyOn(checkoutService, 'continueAsGuest').mockRejectedValue({ status: 429 });
    ...
    it('triggers error callback if customer is unable to continue as guest', async () => {
      jest.spyOn(checkoutService, 'continueAsGuest').mockRejectedValue({ type: 'unknown_error' });

      const handleError = jest.fn();
      const component = mount(
        <CustomerTest onContinueAsGuestError={ handleError } viewType={ CustomerViewType.Guest } />,
      );
  ...
  describe('when view type is "login"', () => {
    ...
    it('handles "sign in" event', async () => {
      jest
        .spyOn(checkoutService, 'signInCustomer')
        .mockReturnValue(Promise.resolve(checkoutService.getState()));
    ...
    it('triggers error callback if customer is unable to sign in', async () => {
      jest.spyOn(checkoutService, 'signInCustomer').mockRejectedValue({ type: 'unknown_error' });
    ...
    it('clears error when "cancel" event is triggered', async () => {
      ...
      jest
        .spyOn(checkoutService, 'clearError')
        .mockReturnValue(Promise.resolve(checkoutService.getState()));
    ...
    it('changes to guest view when "cancel" event is triggered', async () => {
      const handleChangeViewType = jest.fn();
      const component = mount(
        <CustomerTest onChangeViewType={ handleChangeViewType } viewType={ CustomerViewType.Login } />,
      );
  ...
  describe('when view type is "suggested_login"', () => {
    it('renders login form', async () => {
      ...
      expect(component.find(LoginForm).prop('viewType')).toEqual(CustomerViewType.SuggestedLogin);
    });
  });
  ...
  describe('when view type is "enforced_login"', () => {
    it('renders login form', async () => {
      ...
      expect(component.find(LoginForm).prop('viewType')).toEqual(CustomerViewType.EnforcedLogin);
    });
