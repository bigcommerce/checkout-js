    ...
    async (formValues: GuestFormValues) => {
      ...
      try {
        ...
        if (
          ...
          onChangeViewType(CustomerViewType.SuggestedLogin);
        ...
        if (isErrorWithType(error) && error.status === 429) {
          onChangeViewType(CustomerViewType.EnforcedLogin);

          return;
        }

        if (isErrorWithType(error) && error.status === 403) {
          onChangeViewType(CustomerViewType.CancellableEnforcedLogin);
