...
describe('Customer Guest', () => {
  ...
  it('displays error message if privacy policy is required and not checked', async () => {
    ...
    expect(
      screen.getByLabelText(localeContext.language.translate('privacy_policy.required_error')),
    ).toBeInTheDocument();
  ...
  it('enforces login if API returns 429 error', async () => {
    ...
    jest
      ...
      .mockRejectedValue({ type: 'error', status: 429 });
  ...
  it('shows cancellable enforced login if API returns 403 error', async () => {
    ...
    jest
      .spyOn(checkoutService, 'continueAsGuest')
      .mockRejectedValue({ type: 'error', status: 403 });
