...
describe('CustomerInfo', () => {
  ...
  describe('when customer is signed in', () => {
    ...
    it("displays customer's email", () => {
      ...
      expect(component.find('[data-test="customer-info"]').text()).toEqual(getCustomer().email);
    });
    ...
    it('signs out customer when they click on "sign out" button', () => {
      jest
        .spyOn(checkoutService, 'signOutCustomer')
    ...
    it('triggers completion callback if able to sign out', async () => {
      jest.spyOn(checkoutService, 'signOutCustomer').mockResolvedValue(checkoutService.getState());
    ...
    it('triggers error callback if unable to sign out', async () => {
      jest.spyOn(checkoutService, 'signOutCustomer').mockRejectedValue({ type: 'unknown_error' });
