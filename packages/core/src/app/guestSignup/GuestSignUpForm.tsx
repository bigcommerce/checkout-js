...
> = ({ isSigningUp, customerCanBeCreated, passwordRequirements: { minLength } }) => (
  <Fieldset
    legend={
      <Legend>
        <TranslatedString
          id={ customerCanBeCreated ? 'customer.create_account_text' : 'customer.set_password_text' }
        />
      </Legend>
    }
  >
    ...
    validationSchema: ({
      ...
      object({
        password: string()
          ...
          .matches(
            numeric,
            description || language.translate('customer.password_number_required_error'),
          )
          .matches(
            alpha,
            description || language.translate('customer.password_letter_required_error'),
          )
          .min(
            minLength,
            description || language.translate('customer.password_under_minimum_length_error'),
          )
