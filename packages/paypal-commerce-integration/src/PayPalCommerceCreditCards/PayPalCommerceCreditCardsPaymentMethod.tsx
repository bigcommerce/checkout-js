...
const PayPalCommerceCreditCardsPaymentMethod: FunctionComponent<PaymentMethodProps> = (props) => {
  ...
  const { cardCode, showCardHolderName, isHostedFormEnabled, requireCustomerCode } = method.config;
  ...
  const isInstrumentCardNumberRequiredProp = isInstrumentCardNumberRequiredSelector(checkoutState);
  ...
  const getHostedFieldId: (name: string) => string = useCallback(
    (name) => `${compact([method.gateway, method.id]).join('-')}-${name}`,
    [method],
  );
      ...
      return {
        fields: selectedInstrument
          ? {
              ...
              cardExpiryVerification: shouldRenderHostedFields
                ? {
                    accessibilityLabel: language.translate('payment.credit_card_expiry_label'),
        ...
        onValidate: ({ errors = {} }) => {
          forIn(errors, (fieldErrors, fieldType) => {
            ...
            setFieldValue(errorKey, fieldErrors?.[0].type ? fieldErrors[0].type : '');
      ...
      (selectedInstrument) => {
        ...
        return (
          <HostedCreditCardValidation
            ...
            cardExpiryId={shouldRenderHostedFields ? getHostedFieldId('ccExpiry') : undefined}
      ...
      async (options, selectedInstrument) =>
        initializePayment({
          ...
          paypalcommercecreditcards: {
            form: isHostedFormEnabled ? await getHostedFormOptions(selectedInstrument) : undefined,
