...
describe('BigCommercePaymentsCreditCardPaymentMethod', () => {
  ...
  beforeEach(() => {
    ...
    method = { ...getPaymentMethod(), id: 'bigcommerce_payments_creditcards' };
        ...
        <PaymentFormContext.Provider value={{ paymentForm }}>
          <LocaleContext.Provider value={localeContext}>
            <Formik initialValues={initialValues} onSubmit={noop}>
              <BigCommercePaymentsCreditCardsPaymentMethod {...props} />
            </Formik>
  ...
  it('calls initializePayment with correct arguments', async () => {
    ...
    expect(checkoutService.initializePayment).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        bigcommerce_payments_creditcards: expect.any(Object),
      }),
    );
  });
  ...
  it('calls getHostedFormOptions and returns styles with all expected child options', async () => {
    ...
    if (call.bigcommerce_payments_creditcards?.form) {
      const options = await call.bigcommerce_payments_creditcards.form();
  ...
  it('getHostedFormOptions returns correct fields for selectedInstrument with required verifications', async () => {
    ...
    if (call.bigcommerce_payments_creditcards?.form) {
      const options = await call.bigcommerce_payments_creditcards.form(selectedInstrument);
  ...
  it('calls onUnhandledError if error occurs during initializePayment', async () => {
    (checkoutService.initializePayment as jest.Mock).mockRejectedValueOnce(new Error('init error'));
  ...
  it('does not render save card checkbox if vaulting is disabled', () => {
    ...
    expect(screen.queryByText('Save this card for future transactions')).not.toBeInTheDocument();
  });
