...
export default function createMappableInjectHoc<TContextProps>(
  ...
  return <TMappedProps, TOwnProps>(
    ...
    <TProps extends MatchedProps<TMappedProps, TProps>>(
      ...
      const DecoratedComponent: FunctionComponent<Omit<TProps, keyof TMappedProps>> = (props) => {
        ...
        const mapToProps = useMemo(
          () =>
            isMapToPropsFactory(mapToPropsOrFactory) ? mapToPropsOrFactory() : mapToPropsOrFactory,
          [],
        );

        const mappedProps = context ? mapToProps(context, props as unknown as TOwnProps) : context;
      ...
      if (options?.displayNamePrefix && OriginalComponent) {
