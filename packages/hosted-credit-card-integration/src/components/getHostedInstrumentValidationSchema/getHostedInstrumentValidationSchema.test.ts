...
import { type HostedCreditCardValidationValues } from '../../types';

import { getHostedInstrumentValidationSchema } from '.';
...
describe('getHostedInstrumentValidationSchema', () => {
  ...
  it('throws error if card number field is missing', () => {
    ...
    expect(() => schema.validateSync(values)).toThrow('payment.credit_card_number_required_error');
  });

  it('throws error if card number field is invalid', () => {
    values.hostedForm.errors = { cardNumberVerification: 'invalid_card_number' };

    expect(() => schema.validateSync(values)).toThrow('payment.credit_card_number_invalid_error');
  });
  ...
  it('throws error if card number field does not match with stored instrument', () => {
    ...
    expect(() => schema.validateSync(values)).toThrow('payment.credit_card_number_mismatch_error');
  });
  ...
  it('throws error if card expiry field does not match with stored instrument', () => {
    schema = getHostedInstrumentValidationSchema({
      language: language as LanguageService,
      isCardExpiryRequired: true,
    });
