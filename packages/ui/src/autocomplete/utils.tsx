...
export const highlightItem = (item: AutocompleteItem): ReactChild[] | ReactChild => {
  if (!item.highlightedSlices?.length) {
    return item.label;
  }
  ...
  return item.highlightedSlices.reduce((node, slice, i) => {
    ...
    if (notHighlightedLength) {
      node.push(<Fragment key={key}>{label.substr(lastIndex, notHighlightedLength)}</Fragment>);
      key += 1;
    }
...
export const itemToString = (item?: AutocompleteItem | null): string => item?.value || '';
