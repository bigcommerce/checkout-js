...
const BraintreeCreditCardsPaymentMethod: FunctionComponent<PaymentMethodProps> = (props) => {
  ...
  const initializeBraintreePayment: CreditCardPaymentMethodProps['initializePayment'] = useCallback(
        ...
        braintree: {
          ...
          onPaymentError(error: Error) {
            if (error.message === 'THREEDS_VERIFICATION_FAILED') {
              onUnhandledError?.(
                new Error(language.translate('payment.errors.three_d_secure_payment_failed')),
              );
            } else {
              onUnhandledError?.(error);
            }
